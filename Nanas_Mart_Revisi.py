...
def save_receipt_to_csv(name, formatted_date, cart, total_price):
    with open("receipt_database.csv", mode="a", newline="") as file:
        writer = csv.writer(file)
        for item_name, item_price, _ in cart:
            writer.writerow([formatted_date, name, item_name, item_price, total_price])
...
def print_receipt():
    ...
    messagebox.showinfo("Struk Belanja", f"Struk berhasil dicetak ke file '{receipt_path}' dan disimpan di database.")
